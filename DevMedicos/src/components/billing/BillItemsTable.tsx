...
const BillItemsTable: React.FC<BillItemsTableProps> = ({ items, setItems, errors }) => {
  ...
  useEffect(() => {
    ...
    inputRefs.current['item-0']?.current?.focus();
  }, []);
  ...
  const handleKeyDown = (index: number, field: keyof BillItem, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (currentFieldIndex < fields.length - 1) {
        ...
        inputRefs.current[`${fields[currentFieldIndex + 1]}-${index}`]?.current?.focus();
      } else if (index === items.length - 1) {
        // Add new row and focus its first field
        handleAddRow();
        setTimeout(() => {
          inputRefs.current[`item-${index + 1}`]?.current?.focus();
        }, 0);
      } else {
      ...
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {items.map((item, index) => {
            inputRefs.current[`item-${index}`] = useRef<HTMLInputElement>(null);
            inputRefs.current[`quantity-${index}`] = useRef<HTMLInputElement>(null);
              ...
              <tr key={index}>
                <td className="px-4 py-2">
                  <AutocompleteInput
                    ...
                    inputRef={inputRefs.current[`item-${index}`]}
                    onValidSelection={(isValid) => handleValidSelection(index, isValid)}
                  />
                </td>
                <td className="px-4 py-2">
                  <input
                    ref={inputRefs.current[`quantity-${index}`]}
                    type="number"
                    min="0"
                    value={item.quantity || ''}
                ...
                <td className="px-4 py-2">
                  <span className="text-gray-900 font-medium">{formatIndianCurrency(item.total_amount)}</span>
                </td>
                <td className="px-4 py-2">
                  <button
...
//             if (el) inputRefs.current[`item-${index}`] = el;
//           }}
//           onValidSelection={(isValid) => handleValidSelection(index, isValid)}
//         />
//       </td>
...
//             if (el) inputRefs.current[`quantity-${index}`] = el;
//           }}
//           type="number"
//           min="0"
//           value={item.quantity || ''}
